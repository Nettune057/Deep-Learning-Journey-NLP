# coding: utf-8
from util.np import *  # import numpy as np (or import cupy as np)
from util.layers import *
from util.functions import softmax, sigmoid
...
class LSTM:
    def __init__(self, Wx, Wh, b):
        ...
        Parameters
        ----------
        Wx: Weight parameter for input `x` (combines 4 weights)
        Wh: Weight parameter for hidden state `h` (combining 4 weights)
        b: Bias (combining 4 biases)
        '''
        self.params = [Wx, Wh, b]
        self.grads = [np.zeros_like(Wx), np.zeros_like(Wh), np.zeros_like(b)]
        self.cache = None
